...
export function RosterPanel() {
  const addTeam = useCircuit((s) => s.addTeam);
  const removeTeam = useCircuit((s) => s.removeTeam);
  ...
  return (
    <div>
      {/* Add team */}
      <section className="mb-4 rounded-[var(--radius-lg)] border border-border bg-card p-4">
        <h2 className="font-display text-lg font-semibold">Teams</h2>
        <p className="text-sm text-muted-foreground">
          Add a team when the floor grows. Removing a team also removes its drivers and their logs.
        </p>
        <form
              ...
              <div className="flex items-center gap-2">
                ...
                <Button
                  ...
                  onClick={() => {
                    if (
                      confirm(
                        `Remove ${team.name}? Drivers and logs for this team will be deleted.`,
                      )
                    ) {
                      removeTeam(team.id);
                    }
